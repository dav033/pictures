 ...
 *
 *   npx tsx scripts/lora/promover-lora.ts --etiqueta v004-1000 --aprobadas 5
...
function main(): void {
  ...
  if (!fs.existsSync(rutaProc)) {
    console.error(`No hay procedencia para ${etiqueta}. Correr antes scripts/lora/recibir-lora.ts.`);
    process.exit(1);
  }
  ...
  console.log(`\nSi algo sale mal:  npx tsx scripts/lora/promover-lora.ts --etiqueta ${etiqueta} --revertir`);
}
