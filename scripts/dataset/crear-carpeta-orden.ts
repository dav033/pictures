...
// con `npm run ordenes:index` (scripts/dataset/revie-build-index.ts) antes de usar este script.
...
async function main(): Promise<void> {
  ...
  if (!numeroOrden || !/^\d+$/.test(numeroOrden)) {
    console.error("Uso: tsx scripts/dataset/crear-carpeta-orden.ts <numero_de_orden>");
    process.exitCode = 1;
    return;
  }
