...
const whitelist = new Set(
  (getDb().prepare("SELECT id FROM shopify_variante WHERE producto_id = ?").all(PRODUCT_ID) as Array<{ id: string }>).map((row) => row.id),
);
console.log("Despiece calculado:", JSON.stringify(medidas.despiece, null, 2));
console.log("Total globos:", medidas.totalGlobos);

const resuelto = resolverVariantesPorDespiece(PRODUCT_ID, medidas.despiece, whitelist);
console.log("\n--- Resuelto ---");
...
const resuelto2 = resolverVariantesPorDespiece(PRODUCT_ID, [{ tamano: "R-36", pulgadas: 36, cantidad: 5 }], whitelist);
...
const resuelto3 = resolverVariantesPorDespiece("producto-que-no-existe", medidas.despiece, new Set());
