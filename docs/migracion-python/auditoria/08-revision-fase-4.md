...
`razon`/`resumen` en `recomendador.ts` **no tienen límite de longitud ni sanitización** (`RecomendacionSchema`). Un catálogo Happia manipulado puede inducir al modelo a escribir texto libre en `razon`/`resumen`/`mensaje`, devuelto tal cual en la respuesta JSON de `/api/happie/webhook/chat` — consumida directamente por el backend de chat de Happia, es decir, **mostrada a clientes de Happia, no solo al cliente que subió la imagen**. Cualitativamente distinto de la vía imagen porque aquí quien controla el contenido inyectable no es quien ve el resultado.
