...
8. **La puerta física se mide en el globo de la mezcla efectiva.** Los umbrales de globos por metro de `physicalWarningsForPlan` (`estimacion.ts`) están calibrados contra mezclas donde R-12 domina el volumen, así que dejan de ser comparables cuando el punto 1 cambia el globo dominante: un arco 3 × 2,4 m "solo R-24" cuenta 52 globos correctos (8,37/m) donde la mezcla completa contaba 119 (19,16/m), caía por debajo del mínimo de densidad media (8,4/m) y un plan válido dejaba de poder confirmarse —`ESTIMACION_INCONSISTENTE` en `confirmar_plan_decoracion` y error en `/api/generate`—. La banda se escala con `factorGlobosPorMetro`, el mismo modelo que produjo el conteo (diámetro dominante sobre área ponderada del globo, relativo a la mezcla del plan), que vale 1 exacto sin tamaños obligatorios: los planes sin restricción de tamaño ven la puerta de siempre.
...
- **Saltar la rama "demasiado bajo" de la puerta física cuando hay tamaños obligatorios.** Es lo mínimo para dejar de bloquear planes válidos, pero apaga media puerta justo en los planes que más se alejan de la calibración y deja la rama "demasiado alto" con un umbral que tampoco corresponde al globo dominante. Descartada a favor de escalar la banda.
- **Recalibrar los umbrales por mezcla.** Es lo correcto a largo plazo, pero {8, 14, 20} y {48, 68, 88} siguen sin calibrar con montajes reales: inventar una segunda tabla de números no medidos no mejora nada. Escalar con el propio modelo mantiene una sola fuente y deja la calibración pendiente donde estaba.
