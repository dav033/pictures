...
| Vectores dorados | 6 archivos y 40 ocurrencias; ids en `10`, `11`, `12` y `16`; solo nombres en `18` y `19` | Guardan `plan_hash`. Su `expected` ya no se regenera: el modo `--update` de `test-paridad-plan-python.ts` recalculaba el oráculo con el código nuevo y por eso **no** probaba estabilidad, y el paso 3 del ADR-0023 lo retiró (ahora se edita a mano). R1 congela además copias legadas |
...
- **Acción [P] (slice R0):**
  ...
  - reescribir `contracts/README.md` (política de congelamiento y retiro de valores en v1, `DP-06`) y el README de los vectores dorados (cómo se edita a mano un `expected` congelado tras el ADR-0023, y vectores legados de R1).
