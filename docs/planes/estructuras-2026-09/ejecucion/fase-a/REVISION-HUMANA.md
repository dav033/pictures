...
- **APLICADAS el 2026-09-16 en Neon, con autorización del usuario en la sesión:** `024_telemetria_plan_a.sql` y `025_precios_gemini_flash.sql`. Dry-run revisado y después `npm run rag:migrate -- --allow-remote` → "2 migración(es) nueva(s) aplicada(s), 23 ya estaban". Verificado: las 9 columnas nuevas existen y hay 2 filas de precios. Motivo: desde el commit `33878c5` los `INSERT` de `plan_audit_log` y `ai_call_log` incluían esas columnas, así que **entre el 2026-09-15 13:54 y el 2026-09-16 08:4x no se guardó ninguna fila de auditoría ni de telemetría** (ambas absorben errores). Esa base es la misma de producción, así que el despliegue de la rama ya no necesita la migración. Aviso del runner: la migración `020_operational_idempotency.sql` figura aplicada pero ya no existe en el repo.
