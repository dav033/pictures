...
La regeneración RAG v2 quedó verificada sobre PostgreSQL real en Docker y habilitada localmente con `RAG_ENABLED=true`. La ruta obligatoria sigue siendo determinista (parser + FTS/trigram + SQL) y la ruta Gemini/vector quedó validada con key disponible. El bloqueo de integración de generación quedó cerrado: `productIds` enruta explícitamente al catálogo legado SQLite y `ragVariantIds` al catálogo RAG PostgreSQL, sin heurística ni fallback.
...
- HTTP smoke en producción local (puerto `3010`): `/`, `/catalogo`, `/api/productos`, `/api/shopify/sync` y `/api/ia/salud` respondieron `200`; la frontera `/api/generate` sin key terminó en `503 sin_llave` sin llamada pagada y, con Gemini disponible, una generación real PG terminó en `200` con imagen presente.
- Smoke Gemini real: `/api/chat` respondió `200` como SSE con eventos `herramienta`, `texto` y `fin`, sin error; `/api/generate` respondió `200` con `provider=gemini` e imagen presente usando una variante PG. FAL no se utilizó.
...
- E2E no-key: PASS, `0` fallos; p95 final `88.2 ms`, `2` skips opcionales (vector/Gemini sin key), baseline `144.7 ms`, límite baseline+15% `166.4 ms`.
- E2E con Gemini: PASS, `0` fallos, p95 `446.8 ms`; el límite absoluto de `2.100 ms` pasó. El gate de regresión contra baseline no-key quedó `SKIPPED_OPTIONAL` porque los embeddings remotos no son comparables con esa baseline.
...
- E2E con Gemini: `PASS`; sin key la misma capacidad permanece `SKIPPED_OPTIONAL`.
...
| Gemini/vector | PASS | key disponible; `gemini-embedding-2`, 768 dimensiones; E2E vector PASS |

## Decisiones operativas

- `RAG_ENABLED=true`, `RAG_USE_VECTOR=true`, `RAG_USE_FULLTEXT=true`, `RAG_USE_TRIGRAM=true` en el entorno local aprobado; la ruta determinista sigue siendo fallback obligatorio.
...
4. Conservar la corrida `npm run rag:e2e-v2:gemini` cuando haya key; comparar latencia vectorial sólo contra una baseline obtenida con el mismo modo de embeddings.
