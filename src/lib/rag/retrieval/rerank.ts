import type { EventMatchEvidence, RolPresupuesto } from "./types";
...
// Pesos calibrados a mano sobre el catálogo real (§3, Etapa 3a). Siguen sin
// un eval que los ajuste con datos: el que existía (`rag:eval-presupuesto`)
// se fue con el pipeline de franjas (ADR-0023 paso 4) y nunca llegó a ser
// una suite repetible. Documentado aquí para que el próximo ajuste tenga un
// punto de partida explícito en vez de empezar de cero.
const PESO_RANK = 1.0;
...
function ajustePrecio(precio: number, topeCop: number): number {
  ...
  // la de peor calidad visual) ni al límite exacto (deja margen al llamador
  // para sustituir una pieza sin salirse del tope del rol).
