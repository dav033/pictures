import type { EstructuraPlan, Ubicacion } from "./tipos";
...
export function cajasDeEstructuras(estructuras: EstructuraPlan[]): Record<string, UbicacionLayout> {
  const resultado: Record<string, UbicacionLayout> = {};
  const grupos = new Map<Ubicacion, EstructuraPlan[]>();
