...
export async function observarImagenGenerada(sceneSpec: SceneSpec, image: { base64: string; mime: string }, estimate?: DesignMaterialEstimate): Promise<SceneQaObservation | null> {
  ...
  const expected = sceneSpec.elements.map((element) => `${element.element_id}: ${element.name}; placement=${element.target_bbox.x},${element.target_bbox.y},${element.target_bbox.width},${element.target_bbox.height}; quantity=${element.quantity.min}-${element.quantity.max}`).join("\n");
